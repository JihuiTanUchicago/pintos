  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      struct page *kpage = page_allocate (upage, !writable);
      if (kpage == NULL){return false;}//false on space allocation failure
      
      if (page_read_bytes != 0) 
      {
        kpage->exec_file = file;
        kpage->offset = ofs;
...
setup_stack (void **esp, const char* command_line) 
{
  ...
  struct page* kpage = page_allocate (((uint8_t *) PHYS_BASE) - PGSIZE, false);
  if (kpage==NULL) return false;

  kpage->frame = frame_alloc_and_lock (kpage);
  if (kpage->frame==NULL) return false;

  kpage->read_only = false;
  //reading from a file, goes to a file. 
  kpage->swap_or_file = false;

  bool c = install_page (((uint8_t *) PHYS_BASE) - PGSIZE, kpage->frame->base, true);
  ...
  *esp -= sizeof(void*);
  ...
  /* link user virtual address to physical frame in memory for the stack */
  frame_unlock(kpage->frame);
  return true;
}
